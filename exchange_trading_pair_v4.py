...
def test_symbol_exists_and_get_date(exchange, symbol_slash):
    ...
    先檢查最近1天是否有數據且有實際交易量（避免已下架或停止交易的幣種），再獲取歷史上市日期
    ...
    try:
        ...
        try:
            ...
            # 🔥 V4新增：檢查是否有實際交易量
            has_volume = any(candle[5] > 0 for candle in recent_ohlcv)  # candle[5]是成交量
            if not has_volume:
                print(f"    ⚠️  {symbol_slash} 最近1天有K線數據但成交量為0，視為已停止交易")
                return False, None  # 有數據但沒成交量，視為已停止交易
            
            # 如果最近有數據且有交易量，獲取歷史上市日期
            ...
            if symbol_slash.endswith('/USDT'):
                ...
                try:
                    ...
                    if not has_volume:
                        print(f"    ⚠️  {new_symbol} 最近1天有K線數據但成交量為0，視為已停止交易")
                        return False, None
...
def main():
    ...
    for i, row in enumerate(trading_pairs_from_db):
        ...
        for ex_name, markets in all_markets.items():
            ...
            # 🔥 V4修正後的邏輯：所有交易對都必須通過最近1天活躍檢查 + 成交量檢查
            print(f"    🔍 檢查 {ex_name} 的 {symbol} 是否活躍交易且有成交量...")
            ...
            if exists:
                ...
                print(f"    ✅ {ex_name} 支援 {symbol}，最近1天有活躍交易且有成交量，上市日期: {listing_date.date() if listing_date else '未知'}")
                ...
                if trading_pair in markets:
                    print(f"    ❌ {ex_name} 的 {symbol} 在官方清單中但最近1天無活躍交易或無成交量，可能已下架或停止交易")
                else:
    ...
    print("\n🔥 V4版本特色：")
    print("   1. 所有交易對都必須通過最近1天活躍交易檢查")
    print("   2. 必須有實際成交量才會被標記為支持")
    print("   3. 能正確識別技術上存在但已停止交易的幣種（如SNT）")
