...
def test_symbol_exists_and_get_date(exchange, symbol_slash):
    ...
    🔥 V5修正：不使用since參數，直接獲取最新數據來判斷活躍度
        ...
        try:
            ...
            if symbol_slash.endswith('/USDT'):
                ...
                try:
                    ...
                    has_volume = any(candle[5] > 0 for candle in recent_ohlcv)
                    if not has_volume:
                        print(f"    ⚠️  {new_symbol} 有K線數據但成交量為0，視為已停止交易")
                        return False, None
...
def main():
    ...
    for i, row in enumerate(trading_pairs_from_db):
        ...
        for ex_name, markets in all_markets.items():
            ...
            # 🔥 V5修正後的邏輯：所有交易對都必須通過活躍檢查 + 成交量檢查（不依賴系統時間）
            ...
            if exists:
                ...
                print(f"    ✅ {ex_name} 支援 {symbol}，有活躍交易且有成交量，上市日期: {listing_date.date() if listing_date else '未知'}")
                ...
                if trading_pair in markets:
                    print(f"    ❌ {ex_name} 的 {symbol} 在官方清單中但無活躍交易或無成交量，可能已下架或停止交易")
                else:
    ...
    print("\n🔥 V5版本特色：")
    print("   1. 修正系統時間問題，不依賴since參數")
    print("   2. 直接獲取最新數據來判斷活躍度")
    print("   3. 必須有實際成交量才會被標記為支持")
    print("   4. 能正確識別技術上存在但已停止交易的幣種")
